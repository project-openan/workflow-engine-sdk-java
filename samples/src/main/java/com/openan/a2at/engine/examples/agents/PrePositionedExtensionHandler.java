...
/**
 * Handles pre-positioned Authorization-T / Notification-T messages on the
 * agent server side.
 *
 * <p>This is a <b>separate concern</b> from Negotiation-T. Pre-positioned
 * extensions are sent by the orchestrator <i>before</i> the workflow starts
 * (via {@code WorkflowEngineClient.sendExtensionMessage}) to establish
 * whitelists and subscriptions. When an agent receives one, it must:
 * <ol>
 *   <li>Store the payload for later use during business execution</li>
 *   <li>Acknowledge receipt with a short artifact</li>
 *   <li>Complete the task immediately (no negotiation, no business logic)</li>
 * </ol>
 *
 * <p>Extracted from {@code NegotiationBaseAgentExecutor} so that the latter
 * only carries Negotiation-T responsibility.
 */
public class PrePositionedExtensionHandler {
    ...
    public String getAuthorizationPolicy() { return authorizationPolicy; }
    /** The pre-positioned Notification-T subscription text, or null. */
    public String getNotificationSubscription() { return notificationSubscription; }
    /**
     * Detect whether the incoming message carries an Authorization-T or
     * Notification-T extension in its metadata. Returns the extension keyword
     * ("Authorization-T" or "Notification-T"), or null for a normal task.
     */
    ...
    /**
     * Handle a pre-positioned message: store the payload, emit an ACK artifact,
     * and complete the task.
     *
     * @param agentTag  short agent class name for logging
     */
    public void handle(RequestContext ctx, AgentEmitter emitter, String extKeyword, String agentTag) {
        ...
        log.info("[{}] Pre-positioned {} received, payload length={}", agentTag, extKeyword, payloadText.length());
    ...
    private static void emitStatus(AgentEmitter emitter, TaskState state, String contextId,
                                   String taskId, String text, Map<String, Object> metadata) {
