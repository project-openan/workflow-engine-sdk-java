...
public class JdkHttpA2AServer implements AutoCloseable {
    ...
    private RestHandler initServerComponents(
            Map<String, Object> agentCard, AgentExecutor agentExecutor, AgentCard typedCard) {
    ...
    private void handleExchange(HttpExchange exchange, RestHandler restHandler) throws IOException {
        ...
        try {
            ...
            if ("POST".equalsIgnoreCase(method) && "/message:send".equals(path)) {
                var resp =
                        restHandler.sendMessage(buildCallContext(exchange), "", readBody(exchange));
    ...
    private void handleLogin(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            userName =
                    String.valueOf(
                            data.getOrDefault("userName", data.getOrDefault("username", "")));
            password =
                    String.valueOf(data.getOrDefault("value", data.getOrDefault("password", "")));
        } catch (Exception e) {
                    ...
                    new Flow.Subscriber<>() {
                        ...
                        public void onNext(StreamingEventKind item) {
                            try {
                                ...
                                os.write(
                                        formatSse(seq.incrementAndGet(), payload)
