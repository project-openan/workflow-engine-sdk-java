...
public interface WorkflowEngineClient {
    ...
    // One-shot extension message (Authorization-T, Notification-T)
...
For pre-positioning operations (Authorization-T, Notification-T). Generates
metadata value via A2A-T SDK (LLM + prompt template). Bypasses Task-T prompt
generation and Negotiation-T auto-loop.
...
| `extensionUri` | `String` | Full extension URI (metadata key + header value) |
