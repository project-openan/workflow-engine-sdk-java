...
One-shot pre-positioning facade over the same `A2ATransport`. Sends
Authorization-T / Notification-T (and any one-shot extension) to agents
before the workflow starts. Bypasses Task-T prompt generation and the
Negotiation-T auto-loop, and does not emit events through the global
...
| `instruction` | `String` | Short instruction text (becomes `parts[].text`) |
| `naturalLanguageInput` | `String` | Input for SDK prompt generation |
| `extension` | `A2ATExtension` | Extension enum (never hardcode URIs) |

The metadata value is generated by the A2A-T SDK; the engine falls back to
...
Runtime seam for A2A SDK message transport. Implement to customize HTTP
transport behavior.
...
Override to receive real-time execution events. Event types are defined
in `EventType` constants.
...
GET `/api/v1/orchestrate/psop/{psop_id}`. Returns the full workflow
with steps, subtasks, and routing conditions.
...
POST `/api/v1/orchestrate/search`. Returns ranked workflow summaries
matched by natural-language intent.
...
Static factory: `Workflow.fromMap(Map<String, Object>)` parses from
orchestration center API response.
...
- `EventCallback.onEvent` is called from multiple threads (main + SSE
  worker threads). Use synchronization if needed.
...
- Auth failures (401) are logged as `ERROR` and auth headers are not
  set; the request proceeds without auth.
- SSE stream errors after terminal events are logged at `DEBUG` level
  (expected behavior).
