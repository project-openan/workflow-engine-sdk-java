...
The metadata value is generated by the A2A-T SDK; the engine falls back to the raw natural-language input when SDK
...
Reactive hooks interface for agent-pushed A2A-T extension data. Separated from `ControlPoint`:
`ControlPoint` drives the workflow forward (called by the executor); `ExtensionCallback`
reacts to peer-pushed authorization requests and notifications (called by extension handlers).
...
| Method            | When Called                                        | Return                     |
|-------------------|----------------------------------------------------|----------------------------|
| `onAuthorization` | Agent carries Authorization-T in a Task-T response | `Boolean` (true=approve)  |
| `onNotification`  | Agent carries Notification-T in a Task-T response | `Void`                     |
...
> callback parameter, not through `onNotification`. The latter only fires when an agent
> voluntarily includes a Notification-T payload in a `sendMessage` task response.
