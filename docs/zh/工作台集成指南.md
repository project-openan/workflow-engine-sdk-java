...
> 面向广东移动传输工作台智能体研发团队。本文档以 SPN 专线故障跨城协同诊断联创场景为例，从工作台视角说明接入Workflow执行引擎 SDK 的完整路径。
...
# 也可以将密钥作为第二个参数传入
java -cp engine-sdk.jar dev.openan.workflow.engine.client.CredentialCrypto "Admin@123" a1b2c3d4...
```
...
sequenceDiagram
    ...
    participant ES as ExtensionSender (SDK 预定位门面)
