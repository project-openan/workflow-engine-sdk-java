...
```
                    ...
                    (上层业务系统)
                         |
                    A2A-T Task-T 下发原始诊断任务
                         |
          +------------------------------+
          |    传输工作台智能体 (工作台)    |  <-- 本文档的集成方
          |    接入 a2at-engine-java SDK   |
          +------------------------------+
                    /        \
           A2A-T 下发诊断    A2A-T 下发诊断
                /                \
   +------------------+   +------------------+
   | SPN Domain Agent  |   | SPN Domain Agent  |
   | (粤东 OMC, 华为)  |   | (粤西 OMC, 华为)  |
   +------------------+   +------------------+
```
...
```
步骤 1: 引入 Maven 依赖
步骤 2: 准备配置文件 (AgentCard + 凭证 + A2A-T .env)
步骤 3: 实现 AgentExecutor (工作台的 A2A 服务端入口)
步骤 4: 实现 ControlPoint (工作流决策回调)
步骤 5: 组装编排管线 (加载卡片 -> 前置预定位 -> 执行工作流)
步骤 6: (可选) 实现 ExtensionCallback (授权/通知响应钩子)
```
...
```
WAIMO 故障中心          工作台 (AgentExecutor)           SDK 编排管线            OMC (华为)
     |                       |                             |                      |
     | A2A-T Task-T          |                             |                      |
     | (原始诊断任务)        |                             |                      |
     |---------------------->|                             |                      |
     ...
     |                       |               [步骤 1: 加载 AgentCard]            |
     |                       |               [步骤 2: 搜索/加载 PSOP]             |
     |                       |               [步骤 3: 创建 transport + 门面]    |
     |                       |                             |                      |
     |                       |               [步骤 4: 前置预定位]               |
     |                       |               sendAuthorization --------------->| (授权白名单)
     |                       |               <------------------------------- | (授权成功)
     |                       |               sendNotification (订阅) -------->|
     |                       |               <------------------------------- | (订阅成功)
     |                       |                             |                      |
     |                       |               [步骤 5: 执行工作流]               |
     |                       |                             |                      |
     |                       |               step_start: diagnosis_city1        |
     |                       |               onTask -> sendMessage ------------>| Task-T 诊断
     |                       |               <------------------------------ | 诊断结果
     |                       |               (或 Negotiation-T 协商循环)       |
     |                       |               step_complete: diagnosis_city1     |
     |                       |                             |                      |
     |                       |               step_start: diagnosis_city2        |
     |                       |               onTask -> sendMessage ------------>| Task-T 诊断
     |                       |               <------------------------------ | 诊断结果
     |                       |               step_complete: diagnosis_city2     |
     |                       |                             |                      |
     |                       |               step_start: merge (SELF_LOOP)      |
     |                       |               onSelfTask (本地汇总, 无 A2A-T)    |
     |                       |               step_complete: merge               |
     |                       |                             |                      |
     |                       |               complete (工作流结束)               |
     |                       |               onFinish (持久化)                  |
     ...
     |<----------------------|                             |                      |
     |                       |                             |                      |
     |               (后续: OMC 自动抢通并通过 SSE 通道上报结果)                 |
     |                       |               <------------------------------ | Notification-T
     |                       |               onNotification (ExtensionCallback)|
```
