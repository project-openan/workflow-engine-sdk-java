# A2A-T 工作流执行引擎 SDK 工作台集成指南

> 面向广东移动传输工作台智能体研发团队。本文档以 SPN 专线故障跨城协同诊断联创场景为例，从工作台视角说明接入 A2A-T 执行引擎 SDK 的完整路径。读者拿到本文档后，可端到端完成集成。
...
graph TD
    ...
    WB["传输工作台智能体 (工作台)<br/>接入 a2at-engine-java SDK<br/>本文档的集成方"]
...
| Agent 认证 (Bearer 令牌获取/刷新) | 凭证配置 (credentials.json) |
...
graph LR
    ...
    S2["步骤 2<br/>准备配置文件<br/>AgentCard + 凭证 + .env"]
...
| `ExtensionCallback` | 是 (2 个方法，可选) | 接口 + 默认实现 | 响应 OMC 推送的授权请求/通知 |
...
#### 2.2 凭证配置 (credentials.json)

SPN Domain Agent (华为 OMC) 使用 Bearer 认证。SDK 的 `AgentAuthManager` 自动获取并刷新令牌，工作台只需提供凭证:
...
键名必须与 AgentCard 的 `name` 字段一致。SDK 依据此键名匹配 AgentCard 与凭证。密码字段 `value` 使用 `enc:` 前缀标识加密内容，SDK 的 `CredentialCrypto` 自动解密。
...
- `sendNotification`: 建立 SSE 订阅通道。OMC 抢通完成后，通过此通道向工作台推送结果。

这两个方法返回 `CompletableFuture<SendMessageResult>`，`.join()` 阻塞等待完成。SDK 内部自动处理 Authorization-T / Notification-T 报文构造和 Bearer 认证。
...
事件类型完整列表见 SDK 的 `EventType` 常量类。关键事件: `start` -> `step_start` -> `agent_request` / `agent_response` (含 `negotiation_*` / `authorization_*` / `notification`) -> `step_complete` -> `complete` / `error` -> `close`。

### 步骤 6: (可选) 实现 ExtensionCallback

如果工作台需要在 OMC 主动推送授权请求或通知时做出响应 (而非仅在前置预定位阶段单向下发)，可实现 `ExtensionCallback`:
...
注意: `onAuthorization` 和 `onNotification` 是响应 OMC 在 Task-T 响应中携带的扩展数据。前置预定位阶段通过 `ExtensionSender` 下发的授权/订阅是单向推送，不触发这两个回调。两者的区别:

| | 前置预定位 (ExtensionSender) | 响应回调 (ExtensionCallback) |
|---|---|---|
| 发起方 | 工作台主动下发 | OMC 在 Task-T 响答中携带 |
| 时机 | 工作流启动前 | 工作流执行过程中 |
...
sequenceDiagram
    ...
    participant WB as 工作台 (AgentExecutor)
    participant SDK as SDK 编排管线
    ...
    Note over SDK: 步骤 1: 加载 AgentCard
    Note over SDK: 步骤 2: 搜索/加载 PSOP
    Note over SDK: 步骤 3: 创建 transport + 门面

    rect rgb(240, 248, 255)
    Note over SDK,OMC: 步骤 4: 前置预定位
    SDK->>OMC: sendAuthorization (授权白名单)
    OMC-->>SDK: 授权成功
    SDK->>OMC: sendNotification (订阅)
    OMC-->>SDK: 订阅成功
    end

    rect rgb(255, 248, 240)
    Note over SDK,OMC: 步骤 5: 执行工作流

    Note over SDK: step_start: diagnosis_city1
    SDK->>OMC: onTask -> sendMessage (Task-T 诊断)
    OMC-->>SDK: 诊断结果
    Note over SDK,OMC: (或 Negotiation-T 协商循环)
    Note over SDK: step_complete: diagnosis_city1

    Note over SDK: step_start: diagnosis_city2
    SDK->>OMC: onTask -> sendMessage (Task-T 诊断)
    OMC-->>SDK: 诊断结果
    Note over SDK: step_complete: diagnosis_city2

    Note over SDK: step_start: merge (SELF_LOOP)
    Note over SDK: onSelfTask (本地汇总, 无 A2A-T)
    Note over SDK: step_complete: merge

    Note over SDK: complete (工作流结束)
    Note over SDK: onFinish (持久化)
    end

    SDK-->>WB: ExecutionResult

    WB-->>WAIMO: addArtifact(诊断汇总结果)
    WB-->>WAIMO: complete("完成")

    rect rgb(240, 255, 240)
    Note over SDK,OMC: 后续: OMC 自动抢通并通过 SSE 通道上报结果
    OMC-->>SDK: Notification-T (抢通结果)
    Note over SDK: onNotification (ExtensionCallback)
    end
```
...
| `WorkbenchExtensionCallback` (可选) | Java 类 | 继承 `ExtensionCallback`，实现 onAuthorization/onNotification |
| `agentcard/transport_workbench_agent.json` | JSON | 工作台自身的 AgentCard |
| `spn_agent_credentials.json` | JSON | 下游 OMC 的认证凭证 |
...
`spn_agent_credentials.json` 中的密码字段使用 `enc:` 前缀标识加密内容。SDK 的 `CredentialCrypto` 使用对称加密自动解密。加密密钥通过环境变量或配置文件注入。生产环境不应明文存储密码。
...
| `ExtensionCallback` | `onAuthorization` | `CompletableFuture<Boolean> onAuthorization(String, Map)` | OMC 推送授权请求 |
| `ExtensionCallback` | `onNotification` | `CompletableFuture<Void> onNotification(String, Map)` | OMC 推送通知 |
...
| `ExtensionSender` | `sendNotification` | `CompletableFuture<SendMessageResult> sendNotification(String agent, String instruction, String input)` | 前置订阅预定位 |
